  ...
  return (
    ...
    >
      ...
      <div className="project-card-copy">
        <div className="project-meta"><span>{project.number}</span><i style={{ background: project.accent }} />{project.category}</div>
        <h3>{project.name}</h3>
        <h4>{project.subtitle}</h4>
        <p>{project.tagline}</p>
        <div className="project-chips">{project.capabilities.slice(0, 4).map((capability) => <span key={capability}>{capability}</span>)}</div>
        <div className="project-links">
          <Link to={`/projects/${project.id}`}>Explore product <ArrowRight size={16} /></Link>
          {project.demoUrl ? (
            <a href={project.demoUrl} target="_blank" rel="noopener noreferrer">Launch live demo <ArrowUpRight size={15} /></a>
          ) : (
      ...
      <div className="project-visual-wrap">
        <ProjectVisual project={project} compact={false} onOpen={hasScreenshot && onPreview ? () => onPreview(project) : undefined} />
      </div>
